...
from companies.serializers import EmployeeSerializer, AllEmployeesSerializer
...
class Emplyoees(Base):
    ...
    def get(self, request):
        ...
        serializer = AllEmployeesSerializer(employees, many=True)
    ...
    def post(self, request):
        ...
        signup_user = Authentication.signup(
            self, name=name, email=email, password=password, type_account='employee', company_id=enterprise_id)
...
class EmployeeDetail(Base):
    ...
    def put(self, request, employee_id):
        ...
        if groups:
            ...
            for group in groups:
                self.get_group(
                    group_id=group, enterprise_id=employee.enterprise.id)
    ...
    def delete(self, request, employee_id):
        ...
        check_if_owner = User.objects.filter(
            id=employee.user.id, isowner=1).exists()
